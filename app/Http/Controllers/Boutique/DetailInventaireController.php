    ...
    public function store(Request $request)
    {
        ...
        if ($request->isMethod('post') && $request->input('article_id')) {
            ...
            try {
                ...
                $DetailInventaire = DetailInventaire::where([['inventaire_id', $data['inventaire_id']], ['article_id', $data['article_id']], ['unite_id', $data['unite_id']]])->first();
                if($DetailInventaire!=null){
                    ...
                    $detailInventaire->unite_id = $data["unite_id"];
                    $detailInventaire->quantite_en_stocke = $data["quantite_en_stocke"];
                    $detailInventaire->quantite_denombree = $data["quantite_denombree"];
                    $detailInventaire->date_peremption = Carbon::createFromFormat('d-m-Y', $data["date_peremption"]);
       ...
       $jsonData = ["code" => 1, "msg" => "Enregistrement effectué avec succès."];
        ...
        if ($detailInventaire) {
            ...
            try {
                ...
                if(($detailInventaire->article_id!= $data["article_id"]) or ($detailInventaire->unite_id!= $data["unite_id"])) {
                    $DepotArticle = DepotArticle::where([['depot_id', $depot_id], ['article_id', $detailInventaire->article_id], ['unite_id', $detailInventaire->unite_id]])->first();
                ...
                //Ajustement du stock
                $newDepotArticle = DepotArticle::where([['depot_id', $depot_id], ['article_id', $data["article_id"]], ['unite_id', $data["unite_id"]]])->first();
