...
class ArticleApprovisionnementController extends Controller
{
    ...
    public function listeArticlesApprovisionnes($approvisionnement)
    {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->isMethod('post') && $request->input('article_id')) {
            ...
            try {
                ...
                $articleApprovisionnement->unite_id = $data['unite_id'];
                ...
                if($articleApprovisionnement){
                    $depotArticle = DepotArticle::where([['depot_id', $data['depot']], ['article_id', $data['article_id']], ['unite_id', $data['unite_id']]])->first();
                    $mouvementStock = MouvementStock::where([['depot_id', $data['depot']], ['article_id', $data['article_id']], ['unite_id', $data['unite_id']]])->whereDate('date_mouvement', $approvisionnement->date_approvisionnement)->whereDate('date_peremption', Carbon::createFromFormat('d-m-Y', $data['date_peremption']))->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($articleApprovisionnement) {
            ...
            try {
                ...
                $DepotArticle = DepotArticle::where([['depot_id', $depot], ['article_id', $articleApprovisionnement->article_id],['unite_id', $articleApprovisionnement->unite_id]])->first();
                $DepotArticle->quantite_disponible = $DepotArticle->quantite_disponible - $articleApprovisionnement->quantite;
                $DepotArticle->save();
                
                $mouvementStock = MouvementStock::where([['depot_id', $depot], ['article_id', $articleApprovisionnement->article_id], ['unite_id', $articleApprovisionnement->unite_id]])->whereDate('date_mouvement', $approvisionnement->date_approvisionnement)->whereDate('date_peremption',$articleApprovisionnement->date_peremption)->first();
                ...
                if ($articleApprovisionnement) {
                    $depotArticle = DepotArticle::where([['depot_id', $depot], ['article_id', $data['article_id']], ['unite_id', $data['unite_id']]])->whereDate('date_peremption',Carbon::createFromFormat('d-m-Y', $data['date_peremption']))->first();
                    $mouvementStock = MouvementStock::where([['depot_id', $depot], ['article_id', $data['article_id']], ['unite_id', $data['unite_id']]])->whereDate('date_mouvement', $approvisionnement->date_approvisionnement)->whereDate('date_peremption', Carbon::createFromFormat('d-m-Y', $data['date_peremption']))->first();
    ...
    public function articlesRecusByQuantite(){
       ...
       $datas =  Approvisionnement::with('fournisseur')
        ...
        $grandTotalAchat=0; $grandTotalVente=0; $grandTotalQteArticle=0;
        foreach($datas as $data){
           $outPut .= '<tr>
                ...
                $articles = ArticleApprovisionnement::where([['article_approvisionnements.deleted_at', NULL],['article_approvisionnements.approvisionnement_id',$data->id]])
                            ...
                            ->get();
                $totalAchat = 0; $totalVente = 0; $totalQteArticle=0; $total = 0;
