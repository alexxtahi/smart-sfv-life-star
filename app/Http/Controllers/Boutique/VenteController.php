...
class VenteController extends Controller
{
        ...
        if ($request->isMethod('post') && $request->input('vente_id')) {
            ...
            try {
                ...
                if ($data['type_retour'] == "facture_impaye") {
                    ...
                    DB::update('update ventes set impayer = 1, client_id = ?, numero_facture = ? where id = ?', [$data['clients_impaye'], str_replace('TICKET', '', $vente->numero_ticket), $data['vente_id']]);
                    // ! Génération de la facture
                    $this->factureImpayePdf($data['vente_id']);
                    $jsonData["data"] = json_decode($retourArticle);
                    $jsonData["vente_id"] = $data['vente_id'];
                } else {
                    $jsonData["code"] = -1;
                    $jsonData["data"] = null;
    ...
    public function factureImpayeContent($vente)
    {
        ...
        $articlesVentes = ArticleVente::with('article', 'unite')
            ->join('articles', 'articles.id', '=', 'article_ventes.article_id')
            ->leftjoin('param_tvas', 'param_tvas.id', '=', 'articles.param_tva_id')
            ->select('article_ventes.*', 'param_tvas.montant_tva', DB::raw('sum(article_ventes.quantite*article_ventes.prix) as sommeTotale'))
            ->Where([['article_ventes.vente_id', $vente]]) //, ['article_ventes.retourne', 0]])
            ->get();

        //var_dump($articlesVentes);
        ...
        /*foreach ($articlesVentes as $article) {
            $montantTTTC_add = $article['quantite'] * $article['prix'];
        }*/

        //var_dump($montantTTTC_add);
        //var_dump($articlesVentes[0]['quantite']);


        //echo "<script> alert(" . $articlesVentes . "); <script>"; // ! debug

        $content .= '<div class="container-table">
                        <table border="1" cellspacing="-1" width="100%">
                            <tr>
                                <th cellspacing="0" colspan="3" border="2" width="55%" align="center">Numéro du ticket impayé</th>
                                <th cellspacing="0" colspan="3" border="2" width="20%" align="center">Montant TTC</th>
                            </tr>';

        $content .= '<tr>
                        <td style="font-size:20px;"  cellspacing="0" colspan="3" border="2" width="55%">&nbsp;&nbsp;&nbsp;' . $facture['numero_ticket'] . '</td>
                        <td style="font-size:20px;"  cellspacing="0" colspan="3" border="2" align="right" width="20%">' . number_format($montantTTTC_add, 0, ',', ' ') . '&nbsp;&nbsp;&nbsp;</td>
                    </tr>';

        $content .= '<tr>
                        <td style="font-size:20px;"  cellspacing="0" colspan="3" border="2" align="left" width="70%"><b>&nbsp;&nbsp;NET A PAYER</b></td>
                        <td style="font-size:20px;"  cellspacing="0" colspan="3" border="2" align="right" width="30%">&nbsp;&nbsp;<b>' . number_format($montantTTTC_add, 0, ',', ' ') . '</b>&nbsp;&nbsp;&nbsp;</td>
                    </tr>
                </table>
                <p style="font-style: italic;"> NET A PAYER <b>' . ucfirst(NumberToLetter($montantTTTC_add)) . ' FCFA</b></p>
                <p> Fait par <b>' . Auth::user()->full_name . '</b></p>
         </div>';
