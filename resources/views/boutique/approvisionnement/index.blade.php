...
<table id="table" class="table table-warning table-striped box box-primary"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('boutique',['action'=>'liste-approvisionnements'])}}"
               data-unique-id="id"
                                ...
                                <table id="tableAddRowArticle" class="table table-success table-striped box box-success"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="prix_vente_ttc">Prix vente TTC</th>
                                            <th data-field="tva">TVA</th>
                                        </tr>
                                    </thead>
                                </table>
                            </div>
                   ...
                   <input type="text" class="hidden" id="depot"  name="depot"/>
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label>Date de p&eacute;remption </label>
                                <input type="text" class="form-control" name="date_peremption" ng-model="article.date_peremptions" id="date_perimer_add" min="0" placeholder="Date de péremption">
                            </div>
                        </div>
                        <div class="col-md-2">
                                <div class="form-group">
...
<script type="text/javascript">
    ...
    $(function () {
       ...
       $("#searchByFournisseur").change(function (e) { 
            ...
            $("#searchByDate").val("");
            if(fournisseur == 0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('boutique', ['action' => 'liste-approvisionnements'])}}"});
            }
            else{
              $table.bootstrapTable('refreshOptions', {url: '../boutique/liste-approvisionnements-by-fournisseur/' + fournisseur});
            }
        });
       $("#searchByDepot").change(function (e) { 
            ...
            if(depot == 0){
              ...
              $table.bootstrapTable('refreshOptions', {url: '../boutique/liste-approvisionnements-by-depot/' + depot});
            }
        });
       $("#searchByDate").change(function (e) {
            ...
            if(date == ""){
               ...
               $table.bootstrapTable('refreshOptions', {url: '../boutique/liste-approvisionnements-by-date/' + date});
            }
        });
       ...
       $('#date_perimer, #date_perimer_add').datetimepicker({
        ...
        $("#code_barre").keyup(function (e) {
            if(e.which == '10' || e.which == '13') {
                ...
                $('#montant_tva').val("");
                $.getJSON("../parametre/article-by-code/" + code_barre, function (reponse) {
                    if(reponse.total>0){
                        $.each(reponse.rows, function (index, retour) { 
                            $("#article").select2("val",retour.id)
                            $.getJSON("../parametre/find-article/" + retour.id, function (reponse) {
                                if(reponse.total>0){
        ...
        $(".add-row").click(function () {
            if ($("#article").val() != '' && $("#quantite").val() != '' && $("#quantite").val()>0 && $("#unite").val()!='') {
                ...
                //Vérification de l'existance la ligne dans le tableau
                    var articleTrouver = _.findWhere(lotApprovisionnement, {articles: articleId, unites:uniteId, date_peremptions:date_perimer})
                    ...
                    var DataArticle = {'id':idTablle, 'articles':articleId, 'unites':uniteId, 'quantites':quantite,'date_peremptions':date_perimer};
                ...
                $.gritter.add({
                    title: "SMART-SFV",
                    text: "Les champs article, colis et quantité ne doivent pas restés vides et la quantité mini doit être 1.",
                    sticky: false,
    ...
    function updateRow(idApprovisionnement) {
        ...
        $tableArticle.bootstrapTable('refreshOptions', {url: "../boutique/liste-articles-approvisionnes/" + idApprovisionnement});
    ...
    function articleRow(idApprovisionnement) {
        ...
        $tableListeArticle.bootstrapTable('refreshOptions', {url: "../boutique/liste-articles-approvisionnes/" + idApprovisionnement});
