...
<table id="table" class="table table-primary table-striped box box-primary"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('etat',['action'=>'liste-all-ventes-marge'])}}"
               data-unique-id="id"
...
<script type="text/javascript">
       ...
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#dateDebut, #dateFin").change(function (e) {
            ...
            if(dateDebut!='' && dateFin!=''){
                $table.bootstrapTable('refreshOptions', {url: '../etat/liste-all-ventes-marge-periode/' + dateDebut + '/' + dateFin});
            }
            if(dateDebut=='' && dateFin==''){
                $table.bootstrapTable('refreshOptions', {url: "{{url('etat', ['action' => 'liste-all-ventes-marge'])}}"});
            }
        });
         ...
         $("#searchByDepot").change(function (e) { 
            ...
            if(dateDebut=='' && dateFin=='' && depot!=0){
                $table.bootstrapTable('refreshOptions', {url: '../etat/liste-all-ventes-marge-depot/' + depot});
            }
            if(dateDebut!='' && dateFin!='' && depot!=0){
                $table.bootstrapTable('refreshOptions', {url: '../etat/liste-all-ventes-marge-periode-periode-depot/' + dateDebut + '/' + dateFin + '/' + depot});
            }
        });
    ...
    function imprimePdf(){
        ...
        var depot = $("#searchByDepot").val();
        if(dateDebut!='' && dateFin!='' && depot==0){
            window.open("../etat/all-ventes-marge-periode-pdf/" + dateDebut + "/" + dateFin ,'_blank');
        }
        if(dateDebut=='' && dateFin=='' && depot!=0){
            window.open("../etat/all-ventes-marge-depot-pdf/" + depot ,'_blank');
        }
        if(dateDebut!='' && dateFin!='' && depot!=0){
            window.open("../etat/all-ventes-marge-periode-depot-pdf/" + dateDebut + "/" + dateFin + "/" + depot ,'_blank');
        }
        if(dateDebut=='' && dateFin=='' && depot==0){
            window.open("../etat/all-ventes-marge-pdf/",'_blank'); 
        }
    }
