...
<table id="table" class="table table-warning table-striped box box-primary"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('etat',['action'=>'liste-solde-client'])}}"
               data-unique-id="id"
...
<script type="text/javascript">
    ...
    $(function () {
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#searchByClient").change(function (e) { 
            ...
            if(client ==0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('etat', ['action' => 'liste-solde-client'])}}"});
            }else{
                $table.bootstrapTable('refreshOptions', {url: '../etat/liste-solde-client-by-client/' + client});
            }
        });
    });
    ...
    function imprimePdf(){
        var client = $("#searchByClient").val();
        if(client==0){
            window.open("solde-client-pdf/",'_blank');
        }else{
            window.open("solde-client-by-client-pdf/" + client ,'_blank');  
        }
    }
