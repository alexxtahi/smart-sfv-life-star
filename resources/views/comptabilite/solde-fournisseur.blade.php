...
<table id="table" class="table table-warning table-striped box box-primary"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('etat',['action'=>'liste-solde-fournisseurs'])}}"
               data-unique-id="id"
...
<script type="text/javascript">
    ...
    $(function () {
       $table.on('load-success.bs.table', function (e, data) {
        ...
        $("#searchByFournisseur").change(function (e) { 
            ...
            if(fournisseur ==0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('etat', ['action' => 'liste-solde-fournisseurs'])}}"});
            }else{
                $table.bootstrapTable('refreshOptions', {url: '../etat/liste-solde-fournisseur-by-fournisseur/' + fournisseur});
            }
        });
    });
    ...
    function imprimePdf(){
        var fournisseur = $("#searchByFournisseur").val();
        if(fournisseur==0){
            window.open("solde-fournisseur-pdf/",'_blank');
        }else{
            window.open("solde-fournisseur-by-fournisseur-pdf/" + fournisseur ,'_blank');  
        }
    }
 
    function doitFormatter(id, row){
       var doit = row.montantCommande - row.accompteFournisseur;
       if(doit==0){
